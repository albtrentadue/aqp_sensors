...
void loop()
{
  ...
  if (Serial.available() > 0) {
    ...
    if (computerdata[0] == 'c' || computerdata[0] == 'r') time_ = 1800;   //if a command has been sent to calibrate or take a reading
    ...
    if (isDigit(DO_data[0])) {                                            //If the first char is a number we know it is a DO reading, lets parse the DO reading
      string_pars();                                                      
    }
...
void string_pars() {                                                      //this function will break up the CSV string into its 2 individual parts, DO and %sat.
  ...
  for (i = 0; i < 20; i++) {                                              //Step through each char
    if (DO_data[i] == ',') {                                              //do we see a ','
      flag = 1;                                                           //if so we set the var flag to 1 by doing this we can identify if the string being sent
  ...
  /*
  DO_float=atof(DO);                                                      //uncomment this section if you want to take the ASCII values and convert them
  sat_float=atof(sat);                                                    //into a floating point number.
  */
}
